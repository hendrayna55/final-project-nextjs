...
import { Spinner, useToast, Textarea, Button, Grid, Menu, MenuButton, MenuItem, MenuList, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, ModalOverlay, GridItem as ChakraGridItem, Card, Heading, CardBody, Stack, Box, Text, StackDivider } from '@chakra-ui/react';
import { useRouter } from 'next/router';
import { useState, useEffect } from 'react';
import { FaCommentDots, FaHeart, FaSearch, FaHome, FaPlus, FaUser, FaSignOutAlt, FaEllipsisH, FaRegHeart, FaBell } from 'react-icons/fa';
...
export default function ProfileOther() {
    ...
    const otherUser = useQueries({
        prefixUrl: `https://service.pace-unv.cloud/api/user/${id}`,
    });
    console.log(otherUser);
    
    
    const postMe = useQueries({
        ...
        <>
            <div>
                ...
                <div className="flex flex-col min-h-screen bg-japan">
                    ...
                    <main>
                        {postMe?.isLoading || otherUser?.isLoading ? (
                                ...
                                <div className="flex-1 flex justify-center pb-16 mt-20 md:mt-0 z-0">
                                    <div className="w-full max-w-md md:max-w-2xl">
                                        <div className="p-4 md:p-4 space-y-4 md:space-y-3">
                                            {/* Data User */}
                                            <div className="bg-gray-800 bg-opacity-80 rounded-lg shadow-md py-5 px-7 text-white">
                                                <div className='flex justify-between items-center'>
                                                    <div className=''>
                                                        <div
                                                            className="rounded-full w-20 h-20 flex items-center justify-center text-white text-2xl font-bold"
                                                            style={{ backgroundColor: stringToColor(otherUser?.data?.data.name) }}
                                                        >
                                                            {getInitials(otherUser?.data?.data.name)}
                                                        </div>
                                                    </div>

                                                    <div>
                                                        <div className='flex flex-col items-end'>
                                                            <h4 className='text-2xl font-bold'>{otherUser?.data?.data.name}</h4>
                                                            <p className='text-sm'>{otherUser?.data?.data.email}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className='mt-4 grid grid-cols-3'>
                                                    <div className='text-center'>
                                                        <h5 className='text-md font-bold'>Date of Birth</h5>
                                                        <p className='text-sm'>
                                                            {
                                                                otherUser?.data?.data.dob ? 
                                                                new Date(otherUser?.data?.data.dob).toLocaleDateString(
                                                                    'id-ID', 
                                                                    {
                                                                        day: '2-digit',
                                                                        month: 'long',
                                                    ...
                                                    <div className='text-center'>
                                                        <h5 className='text-md font-bold'>Hobby</h5>
                                                        <p className='text-sm'>{otherUser?.data?.data.hobby || '-'}</p>
                                                    </div>
                                                    <div className='text-center'>
                                                        <h5 className='text-md font-bold'>Phone</h5>
                                                        <p className='text-sm'>{otherUser?.data?.data.phone || '-'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className='bg-gray-800 bg-opacity-80 rounded-lg shadow-md py-2 text-white my-3'>
                                                <div className='flex items-center justify-center'>
                                                    <h5 className='text-xl font-bold'>Your Post</h5>
                                                </div>
                                            </div>

                                            {/* Data Post User */}
                                            {
                                                postMe?.data?.data.length > 0 ? (
                                                    <>
                                                        {
                                                            postMe?.data?.data.map((item) => {
                                                    ...
                                                    <div className='bg-gray-800 bg-opacity-80 rounded-lg shadow-md py-2 text-white my-3'>
                                                        <div className='flex items-center justify-center'>
                                                            <h5 className='text-md font-bold'>You have no post</h5>
                                                        </div>
                                                    </div>
                                                )
                                            }
